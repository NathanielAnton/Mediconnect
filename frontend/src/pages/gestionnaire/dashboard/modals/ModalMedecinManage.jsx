...
const ModalProfileView = ({ medecin, onClose }) => {
  ...
  const fetchProfile = async () => {
    ...
    setError("");
    try {
      const response = await axiosInstance.get(`/gestionnaire/medecins/${medecin.id}/profile`);
...
const ModalPlanningEdit = ({ medecin, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchHoraires = async () => {
      try {
        setLoading(true);
        const res = await axiosInstance.get(`/gestionnaire/medecins/${medecin.id}/planning`);
  ...
  const handleSave = async () => {
    ...
    setMessage("");
    try {
      await axiosInstance.put(`/gestionnaire/medecins/${medecin.id}/horaires`, { horaires });
...
const ModalIndisponibiliteEdit = ({ medecin, onClose, onSuccess }) => {
  ...
  const handleAddIndispo = async () => {
    ...
    setMessage("");
    try {
      await axiosInstance.post(`/gestionnaire/medecins/${medecin.id}/indisponibilites`, newIndispo);
