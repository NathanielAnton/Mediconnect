...
const ModalUpdateRendezVous = ({ rendezVous, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchRendezVousData = async () => {
      try {
        setDataLoading(true);
        const response = await axiosInstance.get(`/gestionnaire/rendez-vous/${rendezVous.id}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(
        `/gestionnaire/rendez-vous/${rendezVous.id}`,
        payload
      );
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const response = await axiosInstance.delete(`/gestionnaire/rendez-vous/${rendezVous.id}`);
